...
@NamedQueries({
   ...
   // @NamedQuery(name = "Dictionary.findByDictDateTime", query = "SELECT d FROM Dictionary d WHERE d.dictDateTime = :dictDateTime"),
	...
	@NamedQuery(name = "Dictionary.Allowed", query = "SELECT DISTINCT d, dp FROM Dictionary d, DictionaryPermission dp WHERE d.dictID = dp.dictID AND d.dictDeleted = false and dp.dictPermUser = :user ORDER BY d.dictUser.userLogin, d.dictDateTime DESC, d.dictName"),
